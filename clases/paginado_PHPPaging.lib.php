...
// 		if(!ereg("(^[a-zA-Z0-9]+)$",$var)) return $this->error(true, "El nombre de la variable indicado contiene caracteres no v�lidos");
        if(!preg_replace('/[^A-Za-z0-9_]/', '', $var )) return $this->error(true, "El nombre de la variable indicado contiene caracteres no v�lidos");
	...
	**/
	...
	function div($div) {
		if(empty($div)) return $this->error(true, "El valor indicado en el m�todo linkClase() est� vac�o");
		$this->div = $div;
		return true;
	}
	...
	**/
	function linkClase ($id) {
		...
		if(!preg_replace('/[^A-Za-z0-9_]/', '', $var ) or $id === true) return $this->error(true, "El nombre indicado en el m�todo linkClase() es inv�lido");
		$this->linkClase = $id;
		return true;
	}
	...
	**/
	function fetchNavegacion () {
		...
		if(empty($this->linkEstructura)) {
			...
			$this->linkEstructura = 'https://'.trim($_SERVER['HTTPS_HOST'], '/').'/'.ltrim($_SERVER['PHP_SELF'], '/').'?'.$this->query_string;
		}
		//hector2c
	...
	*/
	function do_link ($page, $title = false,$content) { // Funci�n modificada version 2.0
		...
		$div = $this->div;
		return "<a class='col-lg-2 mr-1 bg-da' style='text-decoration:none' href=\"javascript: fn_paginar('$div', '$href');\"$title$estilo>$content</a>";
	}
	...
	function ejecutar () {
		...
		if($this->numTotalRegistros >= 1) {
			...
			if(!empty($this->sql)) {
				...
				if(!$result) return $this->error(false, @mysql_error(), $sql, preg_replace('/[limit[ ]+[0-9]+(,[ ]*[0-9]+)?]/', '', strtolower($sql) )); // version 2.0

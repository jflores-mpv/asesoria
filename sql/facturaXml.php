...
// ini_set('display_errors', 1);
    ...
    $sesion_empresa_direccion =   $_SESSION['empresa_direccion'];
...
switch($tipoc){
	...
	if(strpos($xml,"<razonSocial>")>0){
		...
		if( $dominio=='dcacorp.com.ec' or $dominio=='www.dcacorp.com.ec'or $dominio=='www.contaweb.ec' 
            or $dominio=='contaweb.ec' or $dominio=='jderp.cloud' or $dominio=='.wwwjderp.cloud'){
					if(substr($tipoIdentificacionComprador,0,2)=='08'){
					    
			    if (strpos($xml, "<incoTermFactura>") !== false) {
			        	$incoTermFactura= substr($xml,strpos($xml,"<incoTermFactura>")+strlen("<incoTermFactura>"),strpos($xml,"</incoTermFactura>")-strlen("<incoTermFactura>")-strpos($xml,"<incoTermFactura>"));
			    }
					    
				if (strpos($xml, "<lugarIncoTerm>") !== false) {
				$lugarIncoTerm= substr($xml,strpos($xml,"<lugarIncoTerm>")+strlen("<lugarIncoTerm>"),strpos($xml,"</lugarIncoTerm>")-strlen("<lugarIncoTerm>")-strpos($xml,"<lugarIncoTerm>"));		      
						  }	
				 if (strpos($xml, "<paisOrigen>") !== false) {
				$paisOrigen= substr($xml,strpos($xml,"<paisOrigen>")+strlen("<paisOrigen>"),strpos($xml,"</paisOrigen>")-strlen("<paisOrigen>")-strpos($xml,"<paisOrigen>"));		      
						  }
				 if (strpos($xml, "<puertoEmbarque>") !== false) {
				$puertoEmbarque= substr($xml,strpos($xml,"<puertoEmbarque>")+strlen("<puertoEmbarque>"),strpos($xml,"</puertoEmbarque>")-strlen("<puertoEmbarque>")-strpos($xml,"<puertoEmbarque>"));		      
						  }
				 if (strpos($xml, "<puertoDestino>") !== false) {
				$puertoDestino= substr($xml,strpos($xml,"<puertoDestino>")+strlen("<puertoDestino>"),strpos($xml,"</puertoDestino>")-strlen("<puertoDestino>")-strpos($xml,"<puertoDestino>"));		      
						  }
				 if (strpos($xml, "<paisAdquisicion>") !== false) {
				$paisAdquisicion= substr($xml,strpos($xml,"<paisAdquisicion>")+strlen("<paisAdquisicion>"),strpos($xml,"</paisAdquisicion>")-strlen("<paisAdquisicion>")-strpos($xml,"<paisAdquisicion>"));		      
						  }
				 if (strpos($xml, "<incoTermTotalSinImpuestos>") !== false) {
				$incoTermTotalSinImpuestos= substr($xml,strpos($xml,"<incoTermTotalSinImpuestos>")+strlen("<incoTermTotalSinImpuestos>"),strpos($xml,"</incoTermTotalSinImpuestos>")-strlen("<incoTermTotalSinImpuestos>")-strpos($xml,"<incoTermTotalSinImpuestos>"));		      
						  }
							
				 if (strpos($xml, "<fleteInternacional>") !== false) {
				    $fleteInternacional= substr($xml,strpos($xml,"<fleteInternacional>")+strlen("<fleteInternacional>"),strpos($xml,"</fleteInternacional>")-strlen("<fleteInternacional>")-strpos($xml,"<fleteInternacional>")); 
				 }	
				 if (strpos($xml, "<seguroInternacional>") !== false) {
				    	$seguroInternacional= substr($xml,strpos($xml,"<seguroInternacional>")+strlen("<seguroInternacional>"),strpos($xml,"</seguroInternacional>")-strlen("<seguroInternacional>")-strpos($xml,"<seguroInternacional>"));
				 }	
							
				 if (strpos($xml, "<seguroInternacional>") !== false) {
				     $gastosAduaneros= substr($xml,strpos($xml,"<gastosAduaneros>")+strlen("<gastosAduaneros>"),strpos($xml,"</gastosAduaneros>")-strlen("<gastosAduaneros>")-strpos($xml,"<gastosAduaneros>"));
				 }			
				 if (strpos($xml, "<seguroInternacional>") !== false) {
				     $gastosTransporteOtros= substr($xml,strpos($xml,"<gastosTransporteOtros>")+strlen("<gastosTransporteOtros>"),strpos($xml,"</gastosTransporteOtros>")-strlen("<gastosTransporteOtros>")-strpos($xml,"<gastosTransporteOtros>"));
				 }			
		   ...
		   $nadic[]=substr($tmpad,strpos($tmpad,"nombre=")+8,strpos($tmpad,">")-1-strpos($tmpad,"nombre=")-8);
				...
				// 		$pdf->Cell(5, 7, '', '', 0, 'L',0);
					$pdf->Cell(80,7, '', '', 1, 'L',0);
						...
						$pdf->SetTextColor(0, 0, 0  );
						$pdf->Cell(15, 7, 'Código Aux', 1, 0, 'C',1);
						$pdf->setFillColor(232, 232, 232 ); 
						$pdf->Cell(10, 7, 'Cant', 1, 0, 'C',1);
						$pdf->setFillColor(232, 232, 232 ); 
						$pdf->Cell(45, 7, 'Descripción', 1, 0, 'C',1);
				...
				// 		$pdf->Cell(13, 7, 'Precio Total', 1, 0, 'R',1);
						...
						$cdet=(mysql_query("select productos.id_producto as  Id,productos.codigo as Codigo,productos.codAux as codigoaux,productos.producto AS Nombre,
				...
				// 		echo "select productos.id_producto as  Id,productos.codigo as Codigo,productos.codAux as codigoaux,productos.producto AS Nombre,
				// 		detalle_ventas.cantidad as Cantidad,detalle_ventas.v_unitario AS Preciounitario, 
				// 		detalle_ventas.descuento as Total,detalle_ventas.detalle as detalle,'' as Nombreproducto from detalle_ventas,productos
				// 		where productos.id_producto=detalle_ventas.id_servicio and detalle_ventas.id_venta=$factura";
                    ...
                    $pdf->startTransaction();
                                ...
                                $lines = $pdf->MultiCell(20, 0, $Odet['Codigo'], 0, 'L', 0, 0, '', '', true, 0, false, true, 0);
                                 ...
                                 $lines = $pdf->MultiCell(20, 0, $Odet['codigoaux'], 0, 'L', 0, 0, '', '', true, 0, false, true, 0);
                            ...
                            $alto=$lines3*3.2;
                                ...
                                //  echo "principal";
						    $pdf->Cell(15, $alto, $Odet['Codigo'],  1, 0, 'L',0);//$pdf->Cell(22, 7, $dcod[$i],  1, 0, 'L',0);
						    
						    $pdf->Cell(15, $alto, $Odet['codigoaux'],  1, 0, 'L',0);//$pdf->Cell(22, 7, $dcod[$i],  1, 0, 'L',0);
							...
							if($lnom<100){
								$pdf->setFillColor(255, 255, 255  );
								$pdf->MultiCell(45, $alto, ''.$Odet['Nombre'], 1, 'L', 1, 0, '', '', true);
							}
...
ventas.Vendedor_id as vendedor
	, ventas.`tipo_inco_term`, ventas.`lugar_inco_term`, ventas.`pais_origen`, ventas.`puerto_embarque`, ventas.`puerto_destino`, 
	ventas.`pais_destino`, ventas.`pais_adquisicion`, ventas.`numero_dae`, ventas.`numero_transporte`, ventas.`flete_internacional`, ventas.`seguro_internaiconal`, ventas.`gastos_aduaneros`, ventas.`gastos_transporte`
...
$numero_transporte =  $row1['numero_transporte'];
   ...
   $sqlFormasPago="SELECT `id`, `id_forma`, `documento`, `id_factura`, `id_empresa`, `valor`, `tipo`, `porcentaje`, `fecha_registro`, `numero_retencion`, `autorizacion`, `intervalo_cuotas` FROM `cobrospagos` WHERE documento='0' AND `id_factura` = '".$factura."'  ORDER BY `cobrospagos`.`id` ASC LIMIT 1";
				...
				$a=array("SIN UTILIZACION DEL SISTEMA FINANCIERO"=>"1","20 OTROS CON UTILIZACION DEL SISTEMA FINANCIERO"=>"2","20 OTROS CON UTILIZACION DEL SISTEMA FINANCIERO"=>"3","CHEQUE"=>"4","DINERO ELECTRONICO"=>"17","TARJETA DE CREDITO"=>"19","20 OTROS CON UTILIZACION DEL SISTEMA FINANCIERO"=>"20");
				foreach($a as $b=>$c)
	    ...
	    while($rowVen = mysql_fetch_array($resultVendedor) ){
	        $pdf->Cell(32, $anchoGeneral, 'Vendedor:',  'LRTB', 0, 'L',0);
  	         ...
  	         //   $resultTrans2=mysql_fetch_array($resultTrans);
	            ...
	            $inicioFila=15;
						if( $dominio=='dcacorp.com.ec' or $dominio=='www.dcacorp.com.ec'or $dominio=='www.contaweb.ec' 
                                or $dominio=='contaweb.ec' or $dominio=='jderp.cloud' or $dominio=='www.jderp.cloud'){
               
					if(substr($tipoIdentificacionComprador,0,2)=='08'){
					  
			
				$pdf->SetX($inicioFila);
				$pdf->Cell(32, $anchoGeneral, 'Tipo IncoTerm :',  'LRTB', 0, 'L',0);
				$pdf->Cell(94, $anchoGeneral, $incoTermFactura,  'LRTB', 1, 'L',0);

			
				$pdf->SetX($inicioFila);
				$pdf->Cell(32, $anchoGeneral, 'Lugar IncoTerm :',  'LRTB', 0, 'L',0);
				$pdf->Cell(94, $anchoGeneral, $lugarIncoTerm,  'LRTB', 1, 'L',0);
			
				
				$sqlPO="SELECT `id_paises`, `descripcion`, `codigo` FROM `paises_destino` WHERE codigo=$paisOrigen";
				$resultPO =mysql_query($sqlPO);
				while($rowPO= mysql_fetch_array($resultPO) ){
				    $nombrePO= $rowPO['descripcion'];
				}
				
				$pdf->SetX($inicioFila);
				$pdf->Cell(32, $anchoGeneral, 'Pais de Origen :',  'LRTB', 0, 'L',0);
					$pdf->Cell(30, $anchoGeneral, $nombrePO,  'LRTB', 0, 'L',0);
				// $fila2= $pdf->GetY();
				
				// $pdf->SetY($fila2);
				// $pdf->SetX($inicioFila);
				$pdf->Cell(25, $anchoGeneral, 'Puerto Embarque:',  'LRTB', 0, 'L',0);
				$pdf->Cell(39, $anchoGeneral, $puertoEmbarque,  'LRTB', 1, 'L',0);
			
				
				// 	$pdf->SetY($fila2);
				// $pdf->SetX($inicioFila);
				// $pdf->Cell(32, $anchoGeneral, 'Pais de Origen :',  'LRTB', 0, 'L',0);
				// 	$pdf->Cell(94, $anchoGeneral, $nombrePO,  'LRTB', 1, 'L',0);
				// $fila2= $pdf->GetY();
				
				// $pdf->SetY($fila2);
				// $pdf->SetX($inicioFila);
				// $pdf->Cell(32, $anchoGeneral, 'Puerto Embarque:',  'LRTB', 0, 'L',0);
				// 	$pdf->Cell(94, $anchoGeneral, $puertoEmbarque,  'LRTB', 1, 'L',0);
				// $fila2= $pdf->GetY();
				
				$sqlPD="SELECT `id_paises`, `descripcion`, `codigo` FROM `paises_destino` WHERE codigo=$pais_destino";
				$resultPD =mysql_query($sqlPD);
				while($rowPD= mysql_fetch_array($resultPD) ){
				    $nombrePD= $rowPD['descripcion'];
				}
				
				$pdf->SetX($inicioFila);
				$pdf->Cell(32, $anchoGeneral, 'Pais Destino:',  'LRTB', 0, 'L',0);
				$pdf->Cell(94, $anchoGeneral, $nombrePD,  'LRTB', 1, 'L',0);
				// $fila2= $pdf->GetY();
				
			
				$pdf->SetX($inicioFila);
				$pdf->Cell(32, $anchoGeneral, 'Puerto Destino :',  'LRTB', 0, 'L',0);
				$pdf->Cell(94, $anchoGeneral, $puertoDestino,  'LRTB', 1, 'L',0);
				// $fila2= $pdf->GetY();
				
				$sqlPA="SELECT `id_paises`, `descripcion`, `codigo` FROM `paises_destino` WHERE codigo=$paisAdquisicion";
				$resultPA =mysql_query($sqlPA);
				while($rowPA= mysql_fetch_array($resultPA) ){
				    $nombrePA= $rowPA['descripcion'];
				}
				
			
				$pdf->SetX($inicioFila);
				$pdf->Cell(32, $anchoGeneral, 'Pais Adquisicion :',  'LRTB', 0, 'L',0);
				$pdf->Cell(94, $anchoGeneral, $nombrePA,  'LRTB', 1, 'L',0);
				// $fila2= $pdf->GetY();
				
					
				$pdf->SetX($inicioFila);
				$pdf->Cell(32, $anchoGeneral, 'Numero DAE :',  'LRTB', 0, 'L',0);
				$pdf->Cell(30, $anchoGeneral, $numero_dae,  'LRTB', 0, 'L',0);
				// $fila2= $pdf->GetY();
				
			
				// $pdf->SetX($inicioFila);
				$pdf->Cell(25, $anchoGeneral, 'Numero transporte :',  'LRTB', 0, 'L',0);
					$pdf->Cell(39, $anchoGeneral, $numero_transporte,  'LRTB', 1, 'L',0);
				// $fila2= $pdf->GetY();
				...
				$pdf->SetX($inicioFila);
			$pdf->MultiCell(32, $anchoGeneral, 'Inco Term Total Sin Impuestos :', 'LRTB', 0, 'L', false);
			
				$pdf->Cell(94, $anchoGeneral, $incoTermTotalSinImpuestos,  'LRTB', 1, 'L',0);
				// $fila2= $pdf->GetY();
					}
...
// 				$pdf->Cell(21, $anchoGeneral,number_format($subtiva0,2), 'LRTB', 1, 'R',0);
            ...
            foreach ($array_ivas as $key => $value) {
				...
				// $filaDescuento= $pdf->GetY();						
		  $dominio = $_SERVER['SERVER_NAME'];
		                    	if( $dominio=='dcacorp.com.ec' or $dominio=='www.dcacorp.com.ec'or $dominio=='www.contaweb.ec'  or $dominio=='contaweb.ec' or $dominio=='jderp.cloud' or $dominio=='www.jderp.cloud'){
		                    	    	
                	if($tipoIdentificacionComprador=='08'){
                	   
                	
		                    	$inicioFila2=141;
				$pdf->SetX($inicioFila2);
				$pdf->Cell(35, $anchoGeneral, 'Flete Internacional :',  'LRTB', 0, 'L',0);
					$pdf->Cell(21, $anchoGeneral, $fleteInternacional,  'LRTB', 1, 'R',0);
			
				$pdf->SetX($inicioFila2);
				$pdf->Cell(35, $anchoGeneral, 'Seguro Internacional :',  'LRTB', 0, 'L',0);
					$pdf->Cell(21, $anchoGeneral, $seguroInternacional,  'LRTB', 1, 'R',0);
			
				
				$pdf->SetX($inicioFila2);
				$pdf->Cell(35, $anchoGeneral, 'Gastos Aduaneros :',  'LRTB', 0, 'L',0);
					$pdf->Cell(21, $anchoGeneral, $gastosAduaneros,  'LRTB', 1, 'R',0);
			
				$pdf->SetX($inicioFila2);
				$pdf->Cell(35, $anchoGeneral, 'Gastos Transporte :',  'LRTB', 0, 'L',0);
				$pdf->Cell(21, $anchoGeneral, $gastosTransporteOtros,  'LRTB', 1, 'R',0);
...
//          INICIO COLUMNA 1 INFORMACION ADICIONAL
    ...
    $fila2= $pdf->GetY();   
            ...
            $a = array("SIN UTILIZACIÓN DEL SISTEMA FINANCIERO" => "01", "OTROS CON UTILIZACIÓN DEL SISTEMA FINANCIERO" => "02", "CON UTILIZACIÓN DEL SISTEMA FINANCIERO" => "03", "CHEQUE" => "04", "DINERO ELECTRÓNICO" => "17", "TARJETA DE CRÉDITO" => "19", "OTROS CON UTILIZACIÓN DEL SISTEMA FINANCIERO" => "20");
            foreach ($a as $b => $c)
                if ($c == $Ofact['FormaPago'])
                    $formp = $b;
	...
	$s .= "<factura id=\"comprobante\" version=\"1.1.0\">\n";		
			...
			$s .= "<infoFactura>\n";
				...
				$s .= "<obligadoContabilidad>".$obc."</obligadoContabilidad>\n";
					$dominio = $_SERVER['SERVER_NAME'];
				if( $dominio=='dcacorp.com.ec' or $dominio=='www.dcacorp.com.ec'or $dominio=='www.contaweb.ec' or $dominio=='contaweb.ec' or $dominio=='jderp.cloud' or $dominio=='.wwwjderp.cloud'){
					if(substr($tipoIdentificacionComprador,0,2)=='08'){
						$s .= "<comercioExterior>EXPORTADOR</comercioExterior>\n";
						$s .= "<incoTermFactura>".strtoupper($row['tipo_inco_term'])."</incoTermFactura>\n";
						$s .= "<lugarIncoTerm>".$row['lugar_inco_term']."</lugarIncoTerm>\n";
						$s .= "<paisOrigen>".$row['origenPais']."</paisOrigen>\n";
						$s .= "<puertoEmbarque>".strtoupper($row['puerto_embarque'])."</puertoEmbarque>\n";
						$s .= "<puertoDestino>".$row['puertopaisdestino']."</puertoDestino>\n";
						$s .= "<paisDestino>".$row['destinoPais']."</paisDestino>\n";
						$s .= "<paisAdquisicion>".$row['adquisicionPais']."</paisAdquisicion>\n";			
					}
				}
				...
				if(substr($tipoIdentificacionComprador,0,2)!='08'){
				$s .= "<direccionComprador>".eliminar_tildes(utf8_decode(substr($row['direccion'],0,300)))."</direccionComprador>\n";
				}
				$s .= "<totalSinImpuestos>".number_format($row['sub_total'], 2, '.', '')."</totalSinImpuestos>\n";
					...
					if(substr($tipoIdentificacionComprador,0,2)=='08'){
						...
						$s .= "<incoTermTotalSinImpuestos>FOB</incoTermTotalSinImpuestos>\n";		
					}
				}
            ...
            or $dominio=='contaweb.ec' or $dominio=='jderp.cloud' or $dominio=='.wwwjderp.cloud'){
				if(substr($tipoIdentificacionComprador,0,2)=='08'){
					$s .= "<fleteInternacional>".number_format($row['flete_internacional'], 2, '.', '')."</fleteInternacional>\n";
					$s .= "<seguroInternacional>".number_format($row['seguro_internaiconal'], 2, '.', '')."</seguroInternacional>\n";
					$s .= "<gastosAduaneros>".number_format($row['gastos_aduaneros'], 2, '.', '')."</gastosAduaneros>\n";
					$s .= "<gastosTransporteOtros>".number_format($row['gastos_transporte'], 2, '.', '')."</gastosTransporteOtros>\n";		
				}
			}
		  ...
		  //    $s .= "<total>".number_format(($row['sub12']-(($row['sub12']*$row['descuento'])/100))*(1+$row['iva']/100)+$row['sub0'], 2, '.', '')."</total>\n";//-(($row['subt0']+$row['subt12'])*$row['Descuento'])/100
			...
			$s .= "</infoFactura>\n";
				 ...
				 productos.codigo,productos.codAux,productos.producto,productos.iva ,detalle_ventas.v_total, detalle_ventas.v_total,  detalle_ventas.total_iva as ivatotal, 
				...
				WHERE detalle_ventas.id_servicio= productos.id_producto AND  impuestos.id_iva = detalle_ventas.tarifa_iva AND detalle_ventas.id_venta='".$id."'");   
 				...
 				while($det=mysql_fetch_array($result1)){
				    ...
				    if($det['codAux']!=''){
				        $s .= "<codigoAuxiliar>".substr($det['codAux'],0,25)."</codigoAuxiliar>\n";
				    }
...
$s .= '<detAdicional nombre="detalle" valor="' . htmlspecialchars($det['detalle'], ENT_XML1 | ENT_COMPAT, 'UTF-8') . '"></detAdicional>' . "\n";
				    $s .= "</detallesAdicionales>\n";
				    }
				    $s .= "<impuestos>\n";		
                  ...
                  //  $Oimpuesto=mysql_fetch_array($resultimpuestos);
					...
					if($det['iva']>0  && $tipoIdentificacionComprador!='08'){
                        ...
                        //     $codigoPorcentaje = 10;
                        // }elseif($Oimpuesto['iva']!=12){
                        //     // Si el IVA no es 12 (y no es 13, según la condición anterior), establecemos el codigoPorcentaje a 8
                        //     $codigoPorcentaje = 8;
                        // }else{
                        //     // Si ninguna de las condiciones anteriores es verdadera, por defecto el IVA es 12, establecemos el codigoPorcentaje a 2
                        //     $codigoPorcentaje = 2;
                        // }

...
if ($vendedor_id!=0 && $vendedor_id!=''){
	...
	while($rowIv = mysql_fetch_array($resulImpuestos) ){
	    ...
	    $listadoImpuestos['total_final_iva'][]= 0;
	    $idiva = $rowIv['id_iva'];
...
if($sesion_id_empresa==41){
	...
	while($row = mysql_fetch_array($result)) //for mayor    for($i=0; $i<$numero_filas; $i++)
	{
	    ...
	    while($rowSub = mysql_fetch_array($resultSubTotalesVta) ){
	        ...
	        $suma_iva += $rowSub['suma_iva'] ;
	    }
	    if($row["ventas_estado"]=='Activo'){
	     	    	$suma_total_iva = $suma_iva;
	    }else{
	     	    	$suma_total_iva = 0;
	     	}
	    if($numero==0){
...
// 		$formasPago = ($row["ventas_id_forma_pago"]==1)?'Efectivo':'Transferencia';
        ...
        for($e =0; $e<$cantidadImpuestos; $e++){
            $idiva = $listadoImpuestos['id_iva'][$e];
    	    if(isset( $listadoImpuestos['total_final_iva'][$e]) ){
    	       
    	        	$data[$filaActual][$idiva]=  number_format( $listadoImpuestos['total_final_iva'][$e], 2, '.', ' ') ;
    	    }else{
...
// 		'retIva'=>number_format($valorRI, 2, '.', ' ')
        ...
        $cantidadData = count($data)-1;
		 for($e =0; $e<$cantidadImpuestos; $e++){
    	    ...
    	    if(isset($listadoSubtotales[$idiva]) && $row["ventas_estado"]=='Activo' ){
    	        $listadoImpuestos['total_final_iva'][$e] += $listadoSubtotales[$idiva];

...
FROM
    ...
    while($row = mysql_fetch_array($result)){ //for mayor    for($i=0; $i<$numero_filas; $i++)
        ...
        FROM
             ...
             plan_cuentas.`id_empresa` = '".$sesion_id_empresa."' 
             AND detalle_libro_diario.`id_plan_cuenta` ='".$detalle_libro_diario_id_plan_cuenta."' 
             AND periodo_contable.`id_periodo_contable`= '".$sesion_id_periodo_contable."' 
             GROUP BY periodo_contable.`id_periodo_contable` ;";
    ...
    //   echo  $sumaDebe."</br>";
        ...
        FROM
             ...
             INNER JOIN `libro_diario` libro_diario ON detalle_libro_diario.`id_libro_diario` = libro_diario.`id_libro_diario`
             AND periodo_contable.`id_periodo_contable` = libro_diario.`id_periodo_contable`
             WHERE libro_diario.`fecha` BETWEEN '".$fecha_desde."' AND '".$fecha_hasta."' 
             AND plan_cuentas.`id_empresa` = '".$sesion_id_empresa."' AND detalle_libro_diario.`id_plan_cuenta` ='".$detalle_libro_diario_id_plan_cuenta."' 
             AND periodo_contable.`id_periodo_contable`= '".$sesion_id_periodo_contable."' GROUP BY  periodo_contable.`id_periodo_contable`;";
            ...
            if($numero_filas == '0'){
                ...
                //*************  IMPRIME LOS VALORES DE LAS CUENTAS DE MOVIMIENTO ****************/
                 ...
                 $data1[] = array(
                    ...
                    'saldo_anterior'=>'',
                    'debito'=>number_format($cadenaDebe[$i], 4, '.', ' '),
                    'credito'=>number_format($cadenaHaber[$i], 4, '.', ' '),
    ...
    $titles2 = array(
        ...
        'saldo_anterior'=>number_format('',4,'.',''),
        'debito'=>number_format($totalDebe,4,'.',''),
        'credito'=>number_format($totalHaber,4,'.',''),
        'saldo'=>number_format($saldototal,4,'.','') //$saldoAcreedor
    );
    ...
    $pdf->ezStartPageNumbers(730, 20, 10);
